...
const ewgl_desktop = 
{
	...
	console:
	{
		...
		{
			ewgl_desktop.cons_elt.style.backgroundColor = bg;
		},
		...
		set_dark_theme: function()
		{
			...
			this.color_def='#FFF';
			ewgl_desktop.cons_elt.style.backgroundColor = '#444B';
		},
		...
		set_light_theme: function()
		{
			...
			this.color_def='#888';
			ewgl_desktop.cons_elt.style.backgroundColor = '#BBBB';
		},
		...
		toggle: function(c)
		{
			...
			ewgl_desktop.console.cons.style.zIndex = (this.internal_on_off) ? 2 : -2;
		}
		, 
...
//				this.custom("<par style='color:"+this.color_def+"'> UNDEFINED </par>");
	...
	label_over: function(label,x,y,col)
	{
		...
		la.style.fontSize = '8';
		ewgl_desktop.canv_cons_elt.appendChild(la);
	},
	...
	launch_desktop: function()
	{
		if (gl === null)
		{
			ewgl_desktop.console.error("Web GL2 is not supported by this browser");
			return;
		}
		this.ortho2D = Mat4();
		this.ortho2D_2 = Mat2();
...
ewgl_common.console = ewgl_desktop.console;

Matrix.ortho2D = function() 
{
	return ewgl_desktop.ortho2D;
}

Matrix.ortho2D_2 = function() 
{
	return ewgl_desktop.ortho2D_2;
}
...
function ewgl_rgb_color(col)
{
	...
	ewgl_desktop.console.info(r,g,b, Vec3(r,g,b));
	return Vec3(r,g,b);
}
...
const UserInterface = 
{
	...
	toggle: function(i)
	{
		...
		ewgl_desktop.interf_elt.style['z-index'] = (ewgl_common.interface_on_off) ? 4 : -4;
		resize_update_wgl();
	},

	set_colors: function(bg,fg)
	{
		ewgl_desktop.is_elt.style.setProperty('--bgcol',bg);
		ewgl_desktop.is_elt.style.setProperty('--fgcol',fg);
	},

	set_dark_theme: function()
	{
		ewgl_desktop.is_elt.style.setProperty('--bgcol','#222A');
		ewgl_desktop.is_elt.style.setProperty('--fgcol','#DDD');
	},

	set_light_theme: function()
	{
		ewgl_desktop.is_elt.style.setProperty('--bgcol','#DDDA');
		ewgl_desktop.is_elt.style.setProperty('--fgcol','#222');
	},
	...
	begin: function(shader_editor=true,show_fps=false)
	{
		...
		ewgl_desktop.interf_elt = document.getElementById("Interface");
		this.internal_parents_widgets.length=0;
		this.internal_parents_widgets.push(ewgl_desktop.interf_elt);
		while (ewgl_desktop.interf_elt.lastChild)
		{
			ewgl_desktop.interf_elt.removeChild(ewgl_desktop.interf_elt.lastChild);
		}
		ewgl_desktop.code_editors.length = 0;
		if (shader_editor)
		{
	...
	{
		// ewgl_desktop.shader_zone_elt.removeChild(ewgl_desktop.fs_she.nextSibling);
		ewgl_desktop.shader_zone_elt.removeChild(ewgl_desktop.fs_she);
		ewgl_desktop.code_editors.pop();
		ewgl_desktop.code_editors.pop();
		ewgl_desktop.fs_she = null;
	},
	...
	add_shader_edit: function(prg)
	{
		let compil_func = () => 
		{
			ewgl_desktop.console.toggle(true);
			ewgl_desktop.console.info_nl('<i>Compilation de '+prg.sh_name+"</i>");
			if (prg.compile())
			{
				ewgl_desktop.console.info_nl(prg.sh_name + ' : compilation OK');
				setTimeout( () => {ewgl_desktop.console.toggle(false); },2000)
			}
		...
		// ewgl_desktop.shader_zone_elt = document.getElementById("ShaderZone");
		this.internal_parents_widgets.push(ewgl_desktop.shader_zone_elt)


		ewgl_desktop.fs_she = this.use_field_set('V','');
		...
		let code_edit_v = CodeMirror(ewgl_desktop.fs_she,{ value:prg.v_src,
			theme: "monokai", 
...
//			comment: true,
		...
		ewgl_desktop.code_editors.push(code_edit_v);
		...
		if (prg.f_src)
		{
			...
			lab.style.color='var(--fgcol);';
			code_edit_f = CodeMirror(ewgl_desktop.fs_she,{ value:prg.f_src,
				theme: "monokai",
				mode: "text/x-glsl-es3",
				indentUnit: 4,
				lineNumbers: true,
...
//				comment: true,
			...
			ewgl_desktop.code_editors.push(code_edit_f);
		}
		else
		{
			lab = this.add_label('no fragment');
			lab.style.color='var(--fgcol';
			ewgl_desktop.code_editors.push(nulls);
		}
...
let MouseManipulator2D_ops =
{
	init()
	{
		...
		ewgl_desktop.canv_cons_elt.addEventListener('mousedown', ev =>
		{
			ev.preventDefault();
			ev.stopImmediatePropagation();
			...
			if (ev.clientX <15)
			{
				...
				if (ev.clientY > ewgl_desktop.canv_cons_elt.clientHeight-15)
				{
					console.log(ev.button);
					if (ev.button==0)
					{
						ewgl_desktop.console.toggle();
					}
					else
					{
						ewgl_desktop.console.clear();
					}
				}
			}
		});
...
let MouseManipulator3D_ops =
{
	...
	init()
	{
		...
		gl.canvas.addEventListener('dblclick', ev =>
		{
			...
			ewgl_desktop.update_needed = true;
		});
		...
		gl.canvas.addEventListener('mouseup', ev =>
		{
			...
			ewgl_desktop.interf_elt_move = false;
			this.console_move = false;
			this.button = -1;
//			if (this.speed > 0.05)
			if ((ewgl_desktop.current_time-this.move_time)<0.1)
			{
				if (this.obj)
				{
		...
		gl.canvas.addEventListener('mousemove',   ev =>
		{
			this.move_time = ewgl_desktop.current_time;
...
let Camera_ops = 
{
	...
	{
		return Matrix.translate(0,0,-this.zcam).mult(Matrix.scale(512)).mult(this.frame.orientation());
	},

	get_matrix_for_skybox: function()
	{
		return this.get_projection_matrix_for_skybox().mult(this.get_view_matrix_for_skybox());
	},
...
{
	ewgl_desktop.scene_camera = Camera();
	ewgl_desktop.scene_camera.set_aspect( gl.canvas.clientWidth/ gl.canvas.clientHeight);
	ewgl_desktop.scene_manip = MouseManipulator3D(ewgl_desktop.scene_camera);
}


function FileDroppedOnCanevas(func)
{
	ewgl_desktop.canv_cons_elt.addEventListener("dragenter", e => 
	{
		e.stopPropagation();
		e.preventDefault();
	}, false);

	ewgl_desktop.canv_cons_elt.addEventListener("dragover",  e => 
	{
		e.stopPropagation();
		e.preventDefault();
	}, false);

	ewgl_desktop.canv_cons_elt.addEventListener("drop", e =>
	{
		e.stopPropagation();
		e.preventDefault();
		const dt = e.dataTransfer;
		const files = dt.files;
		func.apply(null,files);
	 }, false);
}
...
function internal_update_wgl()
{
	requestAnimationFrame( (ts) => 
	{
		...
		ewgl_desktop.current_time = ts/1000.0;
		ewgl_desktop.nb_frames++; 
		if (ewgl_desktop.nb_frames>=20)
		{
			const progress = ts - ewgl_desktop.ts_start;
			ewgl_desktop.fps = 1000*ewgl_desktop.nb_frames /progress;
			ewgl_desktop.ts_start = ts;
			ewgl_desktop.nb_frames = 0;
		}


		if (ewgl_desktop.scene_manip)
		{
			ewgl_desktop.scene_manip.spin_set.forEach((o) =>
			{
				if (o.spin_matrix)
				{
		...
		// if (!ewgl_desktop.pause && (ewgl_desktop.update_needed || ewgl_desktop.continuous_update))
		if ((ewgl_desktop.update_needed || ewgl_desktop.continuous_update))
		{
			ewgl_desktop.update_needed = false;

			if (UserInterface.fps_txt)
			{
				UserInterface.fps_txt.value = ewgl_desktop.fps.toFixed(3) + ' fps';
			}
...
function resize_update_wgl()
{
	...
	ewgl_desktop.ts_start = performance.now();
	ewgl_desktop.update_needed = true;
...
function ui_resize_2d()
{
	...
	ewgl_desktop.ortho2D.data[0] = aspect<1 ? aspect : 1;
	ewgl_desktop.ortho2D.data[5] = aspect<1 ? 1 : 1/aspect;

	ewgl_desktop.ortho2D_2.data[0] = ewgl_desktop.ortho2D.data[0];
	ewgl_desktop.ortho2D_2.data[3] = ewgl_desktop.ortho2D.data[5];
...
function ui_resize_3d()
{
	...
	ewgl_desktop.scene_camera.set_aspect(aspect);

	ewgl_desktop.ortho2D.data[0] = aspect<1 ? 1/aspect : 1;
	ewgl_desktop.ortho2D.data[5] = aspect<1 ? 1 : aspect;
...
{
	let foc = false;
	ewgl_desktop.code_editors.forEach( e =>
	{
		foc |= e.hasFocus();
	});
	if ( !foc )
...
const ewgl = ewgl_desktop;
ewgl.loadRequiredFiles = ewgl_common.loadRequiredFiles;
